...
class QuantizedWf6Af16Linear(DSLinearBase):
    ...
    def transform_param(self, param: torch.Tensor) -> InferenceParameter:
        ...
        #quantized_fake_fp6, scales = self.quantizer(param, num_bits=6, exp_bits=3) HAOJUN
        quantized_fake_fp6 = torch.zeros_like(param)
        scales             = torch.zeros((self.out_channels), dtype=torch.float16, device=param.device)
